...
z_scores = (error_vals - error_mean) / error_std

logger.info("Z-score analysis of prediction errors:")
logger.info(f"Mean of errors: {error_mean:.4f}")
logger.info(f"Standard deviation of errors: {error_std:.4f}")

# Calculate percentages within standard deviation ranges
within_1std = np.mean(np.abs(z_scores) <= 1) * 100
within_2std = np.mean(np.abs(z_scores) <= 2) * 100
within_3std = np.mean(np.abs(z_scores) <= 3) * 100

logger.info(f"Percentage of errors within 1 standard deviation: {within_1std:.2f}% (Expected ~68.27%)")
...
plt.axvline(x=-3, color='g', linestyle='--', alpha=0.7, label='±3σ (99.73%)')
plt.axvline(x=3, color='g', linestyle='--', alpha=0.7)
plt.axvline(x=-2, color='y', linestyle='--', alpha=0.7, label='±2σ (95.45%)')
plt.axvline(x=2, color='y', linestyle='--', alpha=0.7)
plt.axvline(x=-1, color='orange', linestyle='--', alpha=0.7, label='±1σ (68.27%)')
